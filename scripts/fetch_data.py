...
Render / Actions just read these files and return them instantly — zero
heavy computation at request time.
    ...
    with app.app_context():
        ...
        if pp_lines:
            for entry in pp_lines:
                ...
                team_abbr = (
                    (entry.get("team") or player.team_abbr or "").upper()
                )
                ...
                edge = (
                    round(avg_l5 - line, 1) if avg_l5 is not None else None
                )
                streak = hr_l10.get(
                    "streak", {"count": 0, "type": "none"}
                )
                mult = matchup_multiplier(
                    opponent_abbr, primary_stat, opp_defense
                )
                ...
                if (
                    ...
                    and home_hr > away_hr + 0.05
                ):
                    home_away_bonus = 1.0
                elif (
                    current_location == "Road"
                    and away_hr > home_hr + 0.05
                ):
                    home_away_bonus = 1.0
                ...
                conf = confidence_score(
                    ...
                    hit_rate_season=hr_season.get("hit_rate")
                    if "error" not in hr_season
                    else None,
                    edge=edge,
                    matchup_mult=mult,
        ...
        for a, b in combinations(candidates, 2):
            ...
            score = round(
                (a["confidence"] + b["confidence"]) / 2 * penalty, 1
            )
            parlays_2.append(
                {
                    "legs": [a, b],
                    "score": score,
                    "correlated": penalty < 1.0,
                }
            )

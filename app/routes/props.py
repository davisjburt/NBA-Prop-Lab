...
from app.services.hit_rate import (
    hit_rate, hit_rate_combo, COMBO_STATS, STAT_OPP_KEY,
...
from app.services.nba_fetcher import fetch_opponent_defense
import pandas as pd
...
# ── Cached opponent defense (refreshed per request to /api/prizepicks) ──
_opp_defense_cache = {}
...
def prizepicks():
    ...
    for entry in pp_lines:
        ...
        # Extract opponent from most recent matchup
        opponent_abbr = extract_opponent(rows[0].matchup) if rows else None

        # Values for this stat
        if stat in COMBO_STATS:
            cols   = COMBO_STATS[stat]
            values = df[cols].sum(axis=1).tolist()
            primary_stat = cols[0]  # use first component for matchup lookup
        ...
        # Home/away bonus: if player hits better at home and this is a home game
        ...
        current_location = rows[0].location if rows else None
        home_away_bonus  = 0.0
...
def prizepicks_parlays():
    """Returns top 2- and 3-leg parlay suggestions from high-confidence props."""
    from itertools import combinations
    pp_lines = fetch_prizepicks_lines()
    if not pp_lines:
        return jsonify({"error": "Could not fetch PrizePicks data"}), 502

    # Reuse prizepicks logic — fetch the ranked list
    import requests as req
    ranked = req.get("http://localhost:5000/api/prizepicks").json()
    if isinstance(ranked, dict) and "error" in ranked:
        return jsonify(ranked), 502

    # Only consider confident, standard-line picks
    candidates = [p for p in ranked if p["confidence"] >= 65 and p["odds_type"] == "standard"][:30]

    def correlation_penalty(a, b):
        """Penalize legs from same team (correlated outcomes)."""
    ...
    for a, b in combinations(candidates, 2):
        ...
        parlays_2.append({"legs": [a, b], "score": score, "penalty": penalty < 1.0})
    ...
    for a, b, c in combinations(candidates, 3):
        ...
        parlays_3.append({"legs": [a, b, c], "score": score, "penalty": penalty < 1.0})
